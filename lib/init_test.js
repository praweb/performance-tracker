...
const trigger = async (context) => {
  ...
  // const config = await context.config('webpack_config.yml', {default_branch: 'praWeb-patch-4'})
  const config = await context.github.repos.getContents({
    owner: 'praWeb',
    repo: 'webpage-test-results',
    ref: 'test_branch',
  ...
  .then(result => {
    ...
    const config = yaml.load(Buffer.from(result.data.content, 'base64').toString())
    ...
    if (configInfo.valid) {
      ...
      if (configInfo.configLevel === 'Periodic') periodicRunner.run(context, config)
      if (configInfo.configLevel === 'Commit') commitRunner.run(context, config)
...
const fetchTestURL = (context) => {
  return `https://${context.payload.deployment_status.environment}.herokuapp.com`
}
